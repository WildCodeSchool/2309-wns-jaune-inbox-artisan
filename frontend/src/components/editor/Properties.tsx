...
const Properties = () => {
	...
	const handlePropertiesDisplay = () => {
		return (
			<>
				{' '}
				{editedComponnentKeys ? (
					Object?.keys(editedComponnentKeys).map((key) => (
						<PropertyRender
							key={key}
							name={key}
							value={editedComponnentKeys[key]}
						/>
					))
				) : (
					<>No properties</>
							...
							<Flex justify="center" align="center" className="pb-2">
								<Space.Compact block className="pr-2">
									<Button
										icon={<BoldOutlined />}
										type={
											editedComponnentStyle?.fontWeight ? 'primary' : 'default'
										}
									...
									<Button
										icon={<ItalicOutlined />}
										type={
											editedComponnentStyle?.fontStyle ? 'primary' : 'default'
										}
									...
									<Button
										...
										type={
											editedComponnentStyle?.textDecoration
												? 'primary'
												: 'default'
										}
					...
					<>
						<Space direction="vertical">
							<ColorPicker
								...
								showText={(color) => (
									<span>Texte : ({color.toHexString().toUpperCase()})</span>
								)}
								onChange={(_, hex) => {
									dispatch({
										type: 'handleContainerProperty',
							...
							<ColorPicker
								...
								showText={(color) => (
									<span>
										Arrière-plan : ({color.toHexString().toUpperCase()})
									</span>
								)}
							/>
						</Space>
	...
	return (
		<>
			<div>Properties</div>
			{editedComponnentKeys &&
				Object.keys(editedComponnentKeys)?.map((key, index) => (
					<div key={index}>
						{key} :{' '}
						{typeof editedComponnentKeys[key] === 'object'
							? JSON.stringify(editedComponnentKeys[key])
							: editedComponnentKeys[key]}
					</div>
				))}
