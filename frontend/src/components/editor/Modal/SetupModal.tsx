...
type SetupModalPropsType = {
	isOpen: boolean;
	closeModal: () => void;
};

const SetupModal: FC<SetupModalPropsType> = ({ isOpen, closeModal }) => {
	...
	useEffect(() => {
		const layout = [values?.row1, values?.row2, values?.row3].filter(
			(val) => val !== undefined
		);
	...
	return (
		...
		>
			<Alert
				type="warning"
				message="Only do it at initialization of template or you will lose data"
			/>
			<Row gutter={16}>
				<Col className="w-1/2">
					<Form
									...
									rules={[
										{
											required: true,
											message:
												'Veuillez entrer le nombre de colonnes pour la ligne 1!',
										},
									]}
								>
									...
									rules={[
										{
											required: true,
											message:
												'Veuillez entrer le nombre de colonnes pour la ligne 2!',
										},
									]}
								>
									<InputNumber min={1} max={3} defaultValue={1} />
									...
									rules={[
										{
											required: true,
											message:
												'Veuillez entrer le nombre de colonnes pour la ligne 3!',
										},
									]}
								>
									<InputNumber min={1} max={3} defaultValue={1} />
