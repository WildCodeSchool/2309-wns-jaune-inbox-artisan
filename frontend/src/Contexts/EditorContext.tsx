...
const defaultContext = {
	state: [
		...
		[
			{ containerWidth: 8, style: { height: '100%' } },
			{ containerWidth: 8, style: { height: '100%' } },
			{ containerWidth: 8, style: { height: '100%' } },
		],
		[
			{ containerWidth: 12, style: { height: '100%' } },
			{ containerWidth: 12, style: { height: '100%' } },
		],
	],
	dispatch: () => {},
...
export const EditorProvider: FC<{ children: ReactNode }> = ({ children }) => {
	...
	const [editedPostion, setEditedPosition] = useState<EditedPositionType>(
		defaultContext.editedPostion
	);
	...
	return (
		<EditorContext.Provider
			value={{ state, dispatch, editedPostion, handlePosition }}
		>
			{children}
		</EditorContext.Provider>
	);
